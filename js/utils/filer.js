...
var Util = {
  ...
  strToDataURL: function(str, contentType, opt_isBinary) {
    ...
    if (isBinary) {
      return 'data:' + contentType + ';base64,' + self.btoa(str);//No I18N
    } else {
      return 'data:' + contentType + ',' + str;   //No I18N
    }
  },
  ...
  dataURLToBlob: function(dataURL) {
    var BASE64_MARKER = ';base64,';   //No I18N
...
FileError.prototype.__defineGetter__('name', function() {//No I18N
...
var Filer = new function() {
  ...
  var FS_INIT_ERROR_MSG = 'Filesystem has not been initialized.';//No I18N
  var NOT_IMPLEMENTED_MSG = 'Not implemented.';//No I18N
  var NOT_A_DIRECTORY = 'Path was not a directory.'; //No I18N
  var INCORRECT_ARGS = 'These method arguments are not supported.';//No I18N
  var FS_URL_SCHEME = 'filesystem:';//No I18N
  ...
  var getEntry_ = function(callback, var_args) {
    ...
    var onError = function(e) {
      if (e.code == FileError.NOT_FOUND_ERR) {
        if (destStr) {
          throw new Error('"' + srcStr + '" or "' + destStr +'" does not exist.');              //No I18N
        } else {
          throw new Error('"' + srcStr + '" does not exist.'); //No I18N
        }
      } else {
        throw new Error('Problem getting Entry for one or more paths.');//No I18N
      }
    };
    ...
    if ((src.isFile || dest.isDirectory) && dest.isDirectory) {
      ...
      getEntry_(function(srcEntry, destDir) {
        if (!destDir.isDirectory) {
          var e = new Error('Oops! "' + destDir.name + ' is not a directory!');//No I18N
          if (opt_errorHandler) {
            opt_errorHandler(e);
          } else {
            throw e;
  ...
  Filer.prototype.init = function(opt_initObj, opt_successCallback,
    ...
    if (!self.requestFileSystem) {
      throw new MyFileError({
        ...
        name: 'BROWSER_NOT_SUPPORTED'//No I18N
      });
    }
      ...
      rootDir.getDirectory(folders[0], {create: true, exclusive: exclusive},
        function (dirEntry) {
          if (dirEntry.isDirectory) { // TODO: check shouldn't be necessary.
            ...
            var e = new Error(path + ' is not a directory');//No I18N
            if (opt_errorHandler) {
              opt_errorHandler(e);
            } else {
              throw e;
        ...
        function(e) {
          if (e.code == FileError.INVALID_MODIFICATION_ERR) {
            e.message = "'" + path + "' already exists";//No I18N
            if (opt_errorHandler) {
              opt_errorHandler(e);
            } else {
              throw e;
